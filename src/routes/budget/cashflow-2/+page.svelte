<script lang="ts">
    ...
    import cashflowService, { type BudgetGroup, type BudgetAccount, type CashflowEntry } from '$lib/services/cashflowService';
        ...
        supabase.auth.getSession().then(response => {
            if (response?.data?.session?.user) {
                ...
                if (response.data.session) {
                    const expiresAt = response.data.session.expires_at || Math.floor(Date.now() / 1000) + 43200;
    ...
    const sampleGroups: any[] = [
        ...
        { 
            ...
            type: 'income',
            accounts: [
                { id: 'sample-acct-5', name: 'Salary', group_id: 'sample-group-3' }
            ]
        }
    ];
    ...
    const groups = $derived(useSampleData ? sampleGroups : (serverDataAvailable ? serverData.groups : []));
    ...
    const dataSource = $derived(useSampleData ? 'Sample Data' : (serverDataAvailable ? 'Server Data' : 'Sample Data (Server data unavailable)'));
    ...
    async function processRowUpdate(group: string, account: string, week: number, accountId: string, e: any) {
    ...
    function calculateRowTotal(row: TableRow): string {
        ...
        const total = row.data.reduce((sum: number, entry: CashflowEntry | SampleEntry) => 
            sum + parseFloat(entry.value.toString()), 0);
    ...
    async function handleAccountUpdated(event: CustomEvent<{ id: string, name: string, group_id: string }>) {
    ...
    async function saveInlineGroupEdit(originalName: string) {
        ...
        try {
            ...
            if (groupToUpdate) {
                ...
                const updatedGroup = await cashflowService.updateGroup(
                    groupToUpdate.id,
                    inlineGroupName
                );
    ...
    async function handleAccountDelete(account: TableRow) {
        ...
        if (confirm(`Are you sure you want to delete the account "${account.account}"?`)) {
            try {
                ...
                const previousAccount = currentIndex > 0 ? rows[currentIndex - 1].account_id : 
                                       (rows.length > 1 ? rows[1].account_id : null);
    ...
    async function handleGroupDeleteFromMenu(group: any) {
        ...
        if (confirm(`Are you sure you want to delete the group "${group.name}" and all its accounts?`)) {
            try {
...
{:else if authRequired}
    <div class="auth-required">
        ...
        <p>Client authentication status: {clientAuthenticated ? 'Authenticated' : 'Not authenticated'}</p>
        <div class="auth-actions">
            <button class="auth-button" onclick={checkAuthStatus}>Check Auth Status</button>
            <button class="auth-button" onclick={() => window.location.reload()} aria-label="Refresh data by reloading the page" title="Refresh data by reloading the page (faster than internal refresh)">Refresh Page</button>
        </div>
    </div>
                    ...
                    {#each groupedRows as group}
                        {#if group && group.rows}
                            {#each group.rows as row, rowIndex}
                                <tr class="cashflow-row {rowIndex === group.rows.length - 1 ? 'last-in-group' : ''}" data-account-id={row.account_id}>
                                    {#if rowIndex === 0}
                                        ...
                                        {#if editingAccountInline === row.account_id}
                                            <div class="account-edit-form">
                                                ...
                                                <div class="account-edit-actions">
                                                    <button 
                                                        class="save-btn" 
                                                        onclick={() => saveInlineAccountEdit(row.account_id, row.account, row.group)}
                                    ...
                                    {#each weeks as week}
                                        <td class="cell {week === currentWeek ? 'current-week' : ''}">
                                            <input 
                                                type="number" 
                                                class="amount-cell {updatingCell.accountId === row.account_id && updatingCell.week === week ? 'updating' : ''}" 
                                                value={row.data.find((d: { week: number, value: number }) => d.week === week)?.value || 0}
                                                onblur={(e) => processRowUpdate(row.group, row.account, week, row.account_id, e)}
                                                onkeydown={(e) => {
                                                    if (e.key === 'Enter') {
                                                        e.preventDefault();
                                                        ...
                                                        if (previousRow) {
                                                            ...
                                                            const columnIndex = Array.from(currentRow.querySelectorAll('input.amount-cell')).indexOf(currentCell as HTMLInputElement);
    ...
    >
        {#each rows.filter(r => r.account_id === activeAccountMenu) as row}
            ...
            <button 
                ...
                onclick={() => handleAccountDelete(row)}
            >
...
{#if activeGroupMenu !== null}
    ...
    >
        {#each groupedRows.filter(g => g.name === activeGroupMenu) as group}
            ...
            <button 
                class="menu-item delete" 
                onclick={() => handleGroupDeleteFromMenu(group)}
            >
                <MIcon name="delete" color="#ef4444" size="16px" />
                <span>Delete</span>
            </button>
...
{#if showTestModal}
    <div style="position: fixed; top: 0; left: 0; right: 0; bottom: 0; background-color: rgba(0, 0, 0, 0.5); display: flex; align-items: center; justify-content: center; z-index: 10000;">
        <div style="background-color: white; border-radius: 8px; width: 90%; max-width: 800px; max-height: 90vh; overflow-y: auto; box-shadow: 0 4px 20px rgba(0, 0, 0, 0.15); display: flex; flex-direction: column;">
            ...
            <div style="padding: 1rem 1.5rem; background-color: #f3f4f6; border-bottom: 1px solid #e5e7eb;">
                <h3 style="margin: 0; font-size: 1.25rem; color: #1f2937;">Account: {selectedAccountForBudget?.name}</h3>
            </div>
...
<style>
    ...
    .account-menu-popup, .group-menu-popup {
        position: fixed;
        background-color: white;
        border-radius: 8px;
