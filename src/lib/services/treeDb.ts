...
import type {
	CelebrationRow,
	CelebrationInsertRow,
	CelebrationUpdateRow
...
import type {
	ClubMembersRow,
	ClubMembersInsertRow,
	ClubMembersUpdateRow
} from '../types/database.types';
import type {
	TieRelationRow,
	TieRelationInsertRow,
	TieRelationUpdateRow
} from '../types/database.types';
...
export default {
	...
	Person: {
		...
		async create(data: PersonInsertRow): Promise<PersonRow | null> {
			const { data: newData, error } = await supabase.from('person').insert(data).single();
		...
		async all(
			...
			orderby: string = 'first_name',
			isAscending: boolean = true
		): Promise<PersonRow[] | null> {
			const filter = `first_name.ilike.%${filterCriteria}%,middle_name.ilike.%${filterCriteria}%,last_name.ilike.%${filterCriteria}%,maiden_name.ilike.%${filterCriteria}%,alias.ilike.%${filterCriteria}%`;
		...
		async update(id: string, updates: PersonUpdateRow): Promise<PersonRow | null> {
			const { data, error } = await supabase.from('person').update(updates).eq('id', id).single();
	...
	Celebration: {
		async create(data: CelebrationInsertRow): Promise<CelebrationRow | null> {
			const { data: newData, error } = await supabase.from('celebration').insert(data).single();
		...
		async all(): Promise<CelebrationRow[] | null> {
			const { data, error } = await supabase.from('celebration').select('*');
		...
		async single(id: string): Promise<CelebrationRow[] | null> {
			const { data, error } = await supabase.from('celebration').select('*').eq('id', id);
	...
	Tie: {
		async create(data: TieInsertRow): Promise<TieRow | null> {
			const { data: newData, error } = await supabase.from('tie').insert(data).single();
		...
		async all(): Promise<TieRow[] | null> {
			const { data, error } = await supabase.from('tie').select('*');
		...
		async single(id: string): Promise<TieRow[] | null> {
			const { data, error } = await supabase.from('tie').select('*').eq('id', id);
		...
		async update(id: string, updates: TieUpdateRow): Promise<TieRow | null> {
			const { data, error } = await supabase.from('tie').update(updates).eq('id', id).single();
		...
		async Parents(id: string): Promise<{ parent_b: PersonRow[]; parent_c: PersonRow[] } | null> {
			const { data, error } = await supabase.from('tie').select('*').eq('person_a', id);
			...
			const parents: { parent_b: PersonRow[]; parent_c: PersonRow[] } = {
				parent_b: [],
				parent_c: []
			};
		...
		async deleteParent(personId: string, parentId: string): Promise<boolean> {
			return this.deletePartner(personId, parentId);
		},
		async Siblings(
			id: string,
			parent_b: PersonRow,
		...
		async deleteSibling(siblingId: string): Promise<boolean> {
			...
			if (error) {
				...
				console.log('Sibling deleted successfully:', data);
			}
		...
		async Children(
			...
			const results: { partners: { partner: PersonRow; children: PersonRow[] }[] } = {
				partners: []
			};

			if (partners && caller) {
				for (const partner of partners.partner) {
					const childrenData = await db.Tie.Siblings(id, caller, partner);
		...
		relationship: {
			...
			async single(id: string): Promise<TieRelationRow[] | null> {
				const { data, error } = await supabase.from('tie_relationship').select('*').eq('id', id);
		...
		types: {
			async create(data: TieTypeInsertRow): Promise<TieTypeRow | null> {
				const { data: newData, error } = await supabase.from('tie_type').insert(data).single();
			...
			async all(): Promise<TieTypeRow[] | null> {
				const { data, error } = await supabase.from('tie_type').select('*');
			...
			async single(id: string): Promise<TieTypeRow[] | null> {
				const { data, error } = await supabase.from('tie_type').select('*').eq('id', id);
		...
		async insert(tie: TieInsertRow): Promise<TieRow> {
			...
			const { data, error } = await supabase.from('tie').insert(tie).select().single();
	...
	Images: {
		async create(data: ImagesInsertRow): Promise<ImagesRow | null> {
			const { data: newData, error } = await supabase.from('images').insert(data).single();
		...
		async update(id: string, updates: ImagesUpdateRow): Promise<ImagesRow | null> {
			const { data, error } = await supabase.from('images').update(updates).eq('id', id).single();
	...
	Club: {
		async create(data: ClubInsertRow): Promise<ClubRow | null> {
			const { data: newData, error } = await supabase.from('club').insert(data).single();
		...
		async single(id: string): Promise<ClubRow[] | null> {
			const { data, error } = await supabase.from('club').select('*').eq('id', id);
		...
		async update(id: string, updates: ClubUpdateRow): Promise<ClubRow | null> {
			const { data, error } = await supabase.from('club').update(updates).eq('id', id).single();
		...
		member: {
			async create(data: ClubMembersInsertRow): Promise<ClubMembersRow | null> {
				const { data: newData, error } = await supabase.from('member').insert(data).single();
			...
			async all(clubId: number): Promise<ClubMembersRow[] | null> {
				const { data, error } = await supabase.from('member').select('*').eq('club', clubId);
			...
			async update(id: string, updates: ClubMembersUpdateRow): Promise<ClubMembersRow | null> {
				const { data, error } = await supabase.from('member').update(updates).eq('id', id).single();
	...
	EventType: {
		...
		async createEventType(data: EventTypeInsertRow): Promise<EventTypeRow | null> {
			const { data: newData, error } = await supabase.from('event_type').insert(data).single();
		...
		async getEventTypes(): Promise<EventTypeRow[] | null> {
			const { data, error } = await supabase.from('event_type').select('*');
		...
		async deleteEventType(event_name: string): Promise<boolean> {
			const { error } = await supabase.from('event_type').delete().eq('event_name', event_name);
